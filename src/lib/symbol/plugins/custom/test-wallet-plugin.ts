 ...
 * - ダミーデータ生成
 */
export class TestWalletPlugin implements QRGeneratorPlugin {
  readonly id = 'test-wallet';
  readonly name = 'Test & Debug Wallet Plugin';
  ...
  readonly wallet: WalletInfo = {
    id: 'test-wallet',
    name: 'TestWallet',
    displayName: 'Test & Debug Wallet',
    description: '開発・テスト専用ウォレット',
    icon: '/icons/wallets/test-wallet.svg',
    type: 'web',
    supported: true,
    downloadUrl: 'https://example.com/test-wallet',
    deepLinkScheme: 'testwallet://'
  };
  ...
  generateUri(request: PaymentRequest): string {
    const baseUri = 'testwallet://debug';
...
// プラグインインスタンスのエクスポート
export const testWalletPlugin = new TestWalletPlugin();
